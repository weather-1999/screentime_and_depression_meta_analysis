...
def create_forest_plot(df, meta_results, output_prefix):
    ...
    # --- Add right column with numerical estimates (OR [95% CI]) ---
    import matplotlib.transforms as mtransforms

    # Use a blended transform: x in axes-fraction, y in data coordinates
    trans_right = mtransforms.blended_transform_factory(ax.transAxes, ax.transData)

    # Header for the right column (aligned above the top study row)
    ax.text(
        1.02, y_positions[0] + 0.6, "OR [95% CI]",
        transform=trans_right, ha="left", va="bottom", fontsize=10
    )

    # Per-study numeric CI text aligned to each study row
    for i, (es, lower, upper) in enumerate(zip(df["Final_ES"], df["Final_Lower"], df["Final_Upper"])):
        y = y_positions[i]
        ax.text(
            1.02, y, f"{es:.2f} [{lower:.2f}, {upper:.2f}]",
            transform=trans_right, ha="left", va="center", fontsize=9
        )

    # Pooled estimate text in the same right column
    pooled_text = f"{pooled_es:.2f} [{pooled_lower:.2f}, {pooled_upper:.2f}]"
    ax.text(
        1.02, pooled_y, pooled_text,
        transform=trans_right, ha="left", va="center",
        fontsize=9, fontweight="bold"
    )

    # Ensure there is enough room on the right for the text column
    ...
    ax.text(0.02, 0.08, het_text, transform=ax.transAxes, fontsize=10,
    ...
    # Save plots
    plt.savefig(f'{output_prefix}_Forest_ORonly_adjusted_forPublication_2.png', dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.savefig(f'{output_prefix}_Forest_ORonly_adjusted_forPublication_2.pdf', dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    
    print(f"Forest plot saved: {output_prefix}_Forest_ORonly_adjusted_forPublication_2.png")
    print(f"Forest plot saved: {output_prefix}_Forest_ORonly_adjusted_forPublication_2.pdf")
...
def create_funnel_plot(df, meta_results, output_prefix):
    ...
    # Caveat text box (place outside upper-right, below legend; avoids data and funnel shading)
    ax.text(
        1.02, 0.72, caveat_text, transform=ax.transAxes, fontsize=8,
    ...
    # Save plots
    plt.savefig(f'{output_prefix}_Funnel_ORonly_adjusted_forPublication_2.png', dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.savefig(f'{output_prefix}_Funnel_ORonly_adjusted_forPublication_2.pdf', dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    
    print(f"Funnel plot saved: {output_prefix}_Funnel_ORonly_adjusted_forPublication_2.png")
    print(f"Funnel plot saved: {output_prefix}_Funnel_ORonly_adjusted_forPublication_2.pdf")
...
def main():
    ...
    print("="*70)
    print(f"\nOutput files:")
    print(f"  - {output_prefix}_Forest_ORonly_adjusted_forPublication_2.png")
    print(f"  - {output_prefix}_Forest_ORonly_adjusted_forPublication_2.pdf")
    print(f"  - {output_prefix}_Funnel_ORonly_adjusted_forPublication_2.png")
    print(f"  - {output_prefix}_Funnel_ORonly_adjusted_forPublication_2.pdf")
